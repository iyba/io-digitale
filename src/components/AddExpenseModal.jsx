  ...
  return (
    <Sheet onClose={onClose}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {['spesa', 'entrata'].map(t => (
            <button key={t} type="button"
              ...
              style={{
                ...
                border: `1.5px solid ${type === t ? (t === 'spesa' ? '#fb923c' : '#4ade80') : 'rgba(var(--surface-rgb),0.1)'}`,
                background: type === t ? (t === 'spesa' ? 'rgba(251,146,60,0.1)' : 'rgba(74,222,128,0.1)') : 'transparent',
                color: type === t ? (t === 'spesa' ? '#fb923c' : '#4ade80') : 'rgba(var(--text-rgb),0.4)',
        ...
        {/* Category */}
        <div>
          <label style={{ display: 'block', fontSize: '0.75rem', color: 'rgba(var(--text-rgb),0.45)', marginBottom: '0.5rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
            Categoria
          </label>
            ...
            {cats.map(c => {
              ...
              return (
                <button key={c} type="button" onClick={() => setCategory(c)} style={{
                  ...
                  border: `1.5px solid ${active ? s.color : 'rgba(var(--surface-rgb),0.07)'}`,
                  background: active ? s.bg : 'rgba(var(--surface-rgb),0.03)',
                  cursor: 'pointer',
                  display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.35rem',
                  transition: 'all 0.15s',
                }}>
                  <span style={{ fontSize: '1.4rem', lineHeight: 1 }}>{s.icon}</span>
                  <span style={{ fontSize: '0.64rem', fontWeight: active ? 700 : 500, color: active ? s.color : 'rgba(var(--text-rgb),0.35)', textAlign: 'center', lineHeight: 1.2 }}>{c}</span>
                </button>
              )
            })}
          </div>
...
function Sheet({ children, onClose }) {
  return (
    <div onClick={e => { if (e.target === e.currentTarget) onClose() }} style={{
      ...
      <div style={{
        background: 'var(--bg-elev)', border: '1px solid rgba(var(--surface-rgb),0.08)',
