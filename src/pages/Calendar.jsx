  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '0.5rem' }}>
        <div>
          <h1 style={{ margin: 0, fontSize: '1.75rem', fontWeight: 800, color: '#f1f1f8', letterSpacing: '-0.03em', textTransform: 'capitalize' }}>
            {monthName}
          </h1>
          <p style={{ margin: '0.15rem 0 0', fontSize: '0.8rem', color: 'rgba(241,241,248,0.4)' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          {WEEKDAYS.map(d => (
            <div key={d} style={{ textAlign: 'center', padding: '0.6rem 0', fontSize: '0.7rem', fontWeight: 700, color: 'rgba(241,241,248,0.35)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              {d}
            </div>
          ))}
        </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'center' }}>
                  <span style={{
                    ...
                    color: isTodayDate
                      ? 'white'
                      : isCurrentMonth
                        ? isWeekend ? 'rgba(241,241,248,0.5)' : '#f1f1f8'
                        : 'rgba(241,241,248,0.2)',
                  ...
                  <div style={{ display: 'flex', justifyContent: 'center', gap: 2, marginTop: 3, flexWrap: 'wrap' }}>
                    {dayTasks.slice(0, 3).map((t, j) => (
                      <span key={j} style={{
                        ...
                        background: t.completed ? 'rgba(255,255,255,0.2)' : (CAT_COLORS[t.category] || '#94a3b8'),
      ...
      <div style={{ display: 'flex', gap: '0.875rem', flexWrap: 'wrap' }}>
        {Object.entries(CAT_COLORS).map(([cat, color]) => (
          <div key={cat} style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.72rem', color: 'rgba(241,241,248,0.45)' }}>
            <span style={{ width: 7, height: 7, borderRadius: '50%', background: color, display: 'inline-block' }} />
            {cat}
          </div>
        ))}
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.72rem', color: 'rgba(241,241,248,0.45)' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.875rem 1rem', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <p style={{ margin: 0, fontWeight: 700, fontSize: '0.875rem', color: '#f1f1f8' }}>
        ...
        {selectedTasks.length === 0 && selectedExpenses.length === 0 ? (
          <div style={{ padding: '1.5rem 1rem', textAlign: 'center', color: 'rgba(241,241,248,0.25)', fontSize: '0.85rem' }}>
...
function DayTaskRow({ task, onEdit, onToggle }) {
  ...
  return (
    <div style={{
      ...
      borderLeft: `3px solid ${task.completed ? 'rgba(255,255,255,0.1)' : cat}`,
    }}>
      ...
      <div style={{ flex: 1, minWidth: 0, cursor: 'pointer' }} onClick={onEdit}>
        <p style={{ margin: 0, fontSize: '0.875rem', fontWeight: 600, color: '#f1f1f8', textDecoration: task.completed ? 'line-through' : 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {task.title}
        </p>
        <p style={{ margin: '0.1rem 0 0', fontSize: '0.72rem', color: 'rgba(241,241,248,0.4)' }}>
          {task.category}
          {task.time ? ` · ${task.time}` : ''}
...
function DayExpenseRow({ expense }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem 1rem', borderBottom: '1px solid rgba(255,255,255,0.05)', borderLeft: '3px solid rgba(251,146,60,0.5)' }}>
      <span style={{ fontSize: '1rem', flexShrink: 0 }}>{icons[expense.category] || '📦'}</span>
      <p style={{ flex: 1, margin: 0, fontSize: '0.875rem', color: '#f1f1f8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
...
function NavBtn({ onClick, children }) {
  return (
    <button onClick={onClick} style={{
      ...
      color: 'rgba(241,241,248,0.7)', cursor: 'pointer', fontSize: '1.2rem',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
    }}>
      {children}
    </button>
